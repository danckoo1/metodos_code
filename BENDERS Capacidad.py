...
if no_benders:
    ...
    theta = maestro.addVar(vtype=GRB.CONTINUOUS, lb = -GRB.INFINITY)
    ...
    while contador <= NITERACIONES:
        ...
        FO = maestro.objVal
        BOUND = subproblema.objVal + sum(sum(-c[i,j]*(xm[i,j].X) for j in N if j!=i) for i in N)
        print(contador, "/", maestro.objVal, "/", BOUND)
        lista.append(FO)

        # Condicion de quiebre de BENDERE
        if ((contador > 1) and (theta.X -TOL <= subproblema.objVal)): 
