...
print('\n Valor óptimo: %8.4f \n' % valorobjetivo)
print(f" tiempo ejecucion de:{final-inicio}")
...
with open("Tactical.sol", "r", encoding="utf-8") as file:
    i = 0
    for line in file:
        if i>=2:   
            line = line.strip().split(" ")
            name = line[0].split("[")
            var_index = name[1].replace("]", "").split(",")
            name = name[0]
            value = line[1] 
            if name == "Board_production":
                board_production[var_index[1]][var_index[0]] = value
            elif name == "Board_inventory":
                board_inventory[var_index[1]][var_index[0]] = value
            elif name == "Logs_processed":
                logs_processed[var_index[1]][var_index[0]] = value
            else:
                setup[var_index[1]][var_index[0]] = value
        i+=1
from beautifultable import BeautifulTable

table_board_production = BeautifulTable()
table_board_production.set_style(BeautifulTable.STYLE_BOX_ROUNDED)
table_board_production.column_headers =["MONTH\\TABLE"]+BOARD
for m in MONTH:
    row = [m]
    for b in BOARD:
        row.append(board_production[m][b])      
    table_board_production.append_row(row)
print("Table of boards production")
print(table_board_production)

table_board_inventory = BeautifulTable()
table_board_inventory.set_style(BeautifulTable.STYLE_BOX_ROUNDED)
table_board_inventory.column_headers =["MONTH\\TABLE"]+BOARD
for m in MONTH:
    row = [m]
    for b in BOARD:
        row.append(board_inventory[m][b])      
    table_board_inventory.append_row(row)
print("Table of boards inventory")
print(table_board_inventory)

table_log_processed = BeautifulTable()
table_log_processed.set_style(BeautifulTable.STYLE_BOX_ROUNDED)
table_log_processed.column_headers =["MONTH\\TABLE"]+LOG
for m in MONTH:
    row = [m]
    for b in LOG:
        row.append(logs_processed[m][b])      
    table_log_processed.append_row(row)
print("Table of logs processed")
print(table_log_processed)

table_setup = BeautifulTable()
table_setup.set_style(BeautifulTable.STYLE_BOX_ROUNDED)
table_setup.column_headers =["MONTH\\TABLE"]+LOG
for m in MONTH:
    row = [m]
    for b in LOG:
        row.append(setup[m][b])      
    table_setup.append_row(row)
print("table setup")
print(table_setup)
